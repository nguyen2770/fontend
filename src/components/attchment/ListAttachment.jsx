...
export default function ListAttachment({ listDocuments = [] }) {
  ...
  const onSelectAll = (e) => {
    setSelectedFiles(
      e.target.checked ? listDocuments.map((doc) => doc.id) : [],
    );
  };
  ...
  const handlePreview = async (item) => {
    try {
      ...
      const response = await _unitOfWork.resource.previewResource(
        item?.resource?.id,
      );
    ...
    <>
        ...
        extra={
          listDocuments.length > 0 && (
            <Checkbox
              ...
              checked={
                selectedFiles.length === listDocuments.length &&
                listDocuments.length > 0
              }
            >
      ...
      >
        ...
        >
          ...
          >
            {t("workReportByPerson.list_file.download_selected")} (
