...
export default function UpdateAssetMaintenance() {
  ...
  const [assetMaintenanceInfo, setAssetMaintenanceInfo] = useState(null);
  ...
  const [isNullDepreciationType, setIsNullDepreciationType] = useState(false);
  ...
  useEffect(() => {
    if (
      fundingSources &&
      assetModelChange &&
      serial !== assetMaintenanceInfo.serial &&
      (companySetting.createNewUsingAFormula ===
        createNewUsingAFormulaType.healthInsurance ||
        companySetting.createNewUsingAFormula ===
          createNewUsingAFormulaType.healthInsuranceDecree3176)
  ...
  useEffect(() => {
    if (
      fundingSources &&
      assetModelChange &&
      assetMaintenanceInfo.asset?.id !== assetModelChange.asset?.id &&
      (companySetting.createNewUsingAFormula ===
        createNewUsingAFormulaType.healthInsurance ||
        companySetting.createNewUsingAFormula ===
          createNewUsingAFormulaType.healthInsuranceDecree3176)
  ...
  const fetGetAssetMaintenanceById = async () => {
    ...
    if (res && res.code === 1) {
      setAssetMaintenanceInfo(res.data);
      form.setFieldsValue({
        ...res.data,
  ...
  const onFinish = async () => {
    ...
    if (isClone) {
      const res =
        await _unitOfWork.assetMaintenance.createAssetMaintenance(data);
      if (res?.code === 409) {
        if (res?.assetNumber) {
      ...
      const res =
        await _unitOfWork.assetMaintenance.updateAssetMaintenance(data);
      if (res && res.code === 1) {
        ShowSuccess(
          "topRight",
          t("common.notifications"),
          ...
          <Tabs defaultActiveKey="1">
            ...
            <TabPane tab={t("assetModel.model.tabs.documents")} key="4">
              <AssetMaintenanceDocument
                AssetMaintenance={assetMaintenanceInfo}
                isCreate={true}
              />
            </TabPane>
            {/* <TabPane
