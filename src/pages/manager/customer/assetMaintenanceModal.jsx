...
const AssetMaintenanceModal = ({ open, onClose, id, onRefresh }) => {
  ...
  const fetchGetListAssetMaintenance = async () => {
    ...
    const res =
      await _unitOfWork.assetMaintenanceCustomer.getUnassignedAssetMaintenanceCustomer(
        payload,
      );
  ...
  const onSubmit = async () => {
    ...
    const res =
      await _unitOfWork.assetMaintenanceCustomer.createAssetMaintenanceCustomer(
        payload,
      );
    if (res) {
      setSelectedRowKeys([]);
