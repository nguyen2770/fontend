...
export default function CalibrationContract() {
  ...
  useEffect(() => {
    if (page > 1) {
      fetchGetCalibrationContracts(page, searchFilter);
    } else {
      fetchGetCalibrationContracts(1, searchFilter);
    }
  }, [page]);

  const fetchGetCalibrationContracts = async (_page, value) => {
    ...
    if (res && res.code === 1) {
      setAmcs(res?.result?.calibrationContracts);
  ...
  const onUpdate = (value) => {
    navigate(staticPath.updateCalibrationContract + "/" + (value.id || value._id));
  };
  const onDetail = (value) => {
    navigate(staticPath.viewCalibrationContract + "/" + (value.id || value._id));
  };
  const onMapping = (value) => {
    navigate(staticPath.calibrationMappingAssetMaintenance + "/" + (value.id || value._id));
  };
  const onDelete = async (value) => {
    ...
    if (res && res.code === 1) {
      ShowSuccess("topRight", t("common.notifications"), t("common.messages.success.delete"));
    ...
    <div>
              ...
              <Input.Group compact>
                <Select
                  ...
                  options={[
                    ...
                    { value: "contractName", label: t("calibration_contract.contract_name") },
                    { value: "serviceContractorName", label: t("amc.form.service_contractor") },
      ...
      <DrawerSearch
        ...
        onCallBack={(value) => {
          ...
          if (!value.isClose) {
            setPage(1);
            fetchGetCalibrationContracts(1, value);
          }
        }}
