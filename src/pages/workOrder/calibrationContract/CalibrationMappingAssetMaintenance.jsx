...
export default function CalibrationMappingAssetMaintenance() {
  ...
  const onClickDelete = async (_record) => {
    ...
    if (res && res.code === 1) {
      fetchServiceserviceContractorUserMappingss();
      ShowSuccess("topRight", t("common.notifications"), "Xóa thành công");
    } else {
      ShowError("topRight", t("common.notifications"), "Xóa thất bại");
    }
  };
  const onSelectAssetMaintenance = async (_data) => {
    ...
    if (res && res.code === 1) {
      ShowSuccess("topRight", t("common.notifications"), "Thêm liên kết thành công");
      fetchServiceserviceContractorUserMappingss();
    }
  };
  const columns = [
