...
export default function TabsCalibrationWork({ calibrationWorkId }) {
  const { t } = useTranslation();
  const [calibrationWorkDocuments, setCalibrationWorkDocuments] = useState([]);

  useEffect(() => {
    if (calibrationWorkId) {
      fetchGetSchedulePreventiveDocumentsBySchedulePreventive();
    }
  }, [calibrationWorkId]); // eslint-disable-line react-hooks/exhaustive-deps

  const fetchGetSchedulePreventiveDocumentsBySchedulePreventive = async () => {
    let res =
      await _unitOfWork.calibrationWorkDocuments.getCalibrationWorkDocumentsByCalibrationWorkId(
        { calibrationWorkId: calibrationWorkId },
      );
    if (res && res.code === 1) {
      setCalibrationWorkDocuments(res?.calibrationWorkDocuments);
    }
  };
  ...
  const onClickDelete = async (record) => {
    let res =
      await _unitOfWork.calibrationWorkDocuments.deleteCalibrationWorkDocuments(
        record?._id || record?.id,
      );
    if (res && res.code === 1) {
      ShowSuccess(
        "topRight",
        t("calibrationWork.calibration_job_managements"),
  ...
  const onUploadFile = async ({ file }) => {
    ...
    const res =
      await _unitOfWork.calibrationWorkDocuments.createCalibrationWorkDocuments(
        {
          resource,
  ...
  return (
    <div className="">
      <div className="mb-2 flex mt-3" style={{ textAlign: "end" }}>
        <Upload
      ...
      <TabAttachment listDocuments={calibrationWorkDocuments} />

      {/* <Table
        rowKey="id"
        columns={columns}
        dataSource={calibrationWorkDocuments}
        bordered
        pagination={false}
      /> */}
    </div>
